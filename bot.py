...
while True:
    '''
    report_mess = 0 # отчет об ошибке
    def report(report_mess):
     report_mess = report_mess + 1
     return report_mess
     '''
    try:
        try:
            ...
            bot = telebot.TeleBot('1953653900:AAEegHZHdAB3RYX_ZltfJAkUttTRM2oREds')
            
            test_st = True # <<<<<<<<<<<<<<<<<<<<<<

            if test_st == True:
                a_s_ru = "/home/ubuntu/wlsel.txt" 
                a_k_ru = "/home/ubuntu/wlkam.txt"
                a_h_ru = ""
                a_m_ru = "" 
                a_kb_ru = ''
                a_mt_ru = ''
                a_tz_ru = '/home/ubuntu/wltaz.txt'
                a_kt_ru = "" 
                a_im_ru = ""
                a_lg1_ru = ""  
                a_s_en = ''
                ...
                a_s_ru = "/home/silvertine/Documents/wlsel.txt" 
                a_k_ru = "/root/selkupbot/wlkam.txt"
                a_h_ru = "/root/selkupbot/wlhant.txt"
                a_m_ru = "/root/selkupbot/wlmans.txt" 
                a_kb_ru = '/root/selkupbot/wlkoj.txt'
                a_mt_ru = '/root/selkupbot/wlmat.txt'
                a_tz_ru = '/root/selkupbot/wltaz.txt'
                a_kt_ru = "/root/selkupbot/wlket.txt"
                a_im_ru =  "/root/selkupbot/wlitm.txt"
                a_lg1_ru = ''
                a_s_en = '/root/selkupbot/wlselen.txt'
                a_k_en = '/root/selkupbot/wlkamen.txt'
                a_h_en = ''
                a_m_en = ''
                a_kb_en = ''
                a_mt_en = ''
                ...
                a_im_en = ''
                a_lg1_en = ''
                a_s_fi = '/root/selkupbot/wlselfi.txt'
                a_k_fi = ''
                a_h_fi = ''
                a_m_fi = ''
                a_kb_fi = ''
        ...
        ###
            ...
            # здесь можно загрузить перем. 'a' при смене раскладки
                ...
                global workdict, a, a_lang, strct, slenwd, wdct
                ...
                with open(str(a), "r") as a:
                    for line in a:
                        ...
                        strcount.append('a')
                strct = len(strcount)
                print("! strings = " + str(strct))
                slenwd = str(len(workdict))
                wdct = workdict.count('\n') + 1
                print('! str count = ' + slenwd)
                a.close()
            ...
            def start_message(message):
                ...
                markup.add(telebot.types.InlineKeyboardButton(text='нарымский диалект', callback_data='sel'))
                markup.add(telebot.types.InlineKeyboardButton(text='шёшкупский диалект', callback_data='kam'))
                markup.add(telebot.types.InlineKeyboardButton(text='верхнеобской диалект', callback_data='taz'))
                ...
                bot.send_message(message.chat.id, text=
                    "Выберите диалект для перевода", reply_markup=markup)

                bot.send_message(message.chat.id, text=
                    "Напишите в чат слово для перевода.")
            ...
            def query_handler(call):	
                ...
                global addwrd_key, a_mod, a_stat, resultcnt, actcnt, strct, strcount
                ...
                if call.data == "sel":
                    ...
                    workdictmaking()
                    a_stat = 'нарымский южноселькупский'
                    x = '### keyboard changed > narym\n'
                    xj(x)
                    print(x)
                    ...
                    workdictmaking()
                    a_stat = 'шёшкупский южноселькупский'
                    ...
                    workdictmaking()
                    a_stat = 'верхнеобской южноселькупский'
                    x = '### keyboard changed > upperob\n'
                    xj(x)
                    print(x)
            ...
            def start_message(message):
                ...
                global a_stat, resultcnt, actcnt, test_st, current_datetime, timetaken
                ...
                try:
                    ...
                    try:
                        bot.send_message(message.from_user.id, 'Keyboard = ' + str(a_test) + '\n\nKey = ' + str(a_mod) + '\nAddword key = ' + str(addwrd_key) + '\nLang = ' + str(a_lang) + 
                            '\nResult count = ' + str(resultcnt) + '\nTesting mode = ' + str(test_st)  + '\nПереводов за сессию = ' + str(actcnt)  + '\nВремя поиска = ' + str(timetaken) + '\nСессия = ' + str(current_datetime))
            ...
            def start_message(message):
                ...
                bot.send_message(message.chat.id, 'Командой /lang вы можете выбрать язык перевода, /set - настройки бота и /stat - состояние бота\nВведите слово, которое нужно перевести на ' + str(a_stat))
            ...
            def get_text_messages(message):
                global a, a_lang, a_stat, resultcnt, actcnt, repltxt, wdct, strct, timetaken

                if message.text == "/help":
                    bot.send_message(message.from_user.id, 'If the bot is not working > @lattar_bot < Если бот не работает'
                        'По всем вопросам >>> @rokuntew')
                    x = "### help message"
                    xj(x)
                    print(x)


                elif '+addword' in message.text:          # добавляем слова в словарь 
                    ...
                    try:
                        ...
                        mtext = mtext.lower() # строчные буквы
        ...
        ############# 
                            ...
                            bot.send_message(message.from_user.id, "Найдено в словаре: \n")

                            try:
                                try:
                                    bot.send_message(message.from_user.id, "[" + str(a_stat) + ', строк: ' + str(strct) + ']')
                        ...
                        if a_stat == 'not defined':
                            bot.send_message(message.from_user.id,'Язык перевода не выбран! Введите /lang')
                        else:
